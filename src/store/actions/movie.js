...
export const getMovieDetailCredit = movieID => {
  return async (dispatch, getState) => {
    try {
      const response = await API.getMovieDetailCredit(movieID);
      // console.log("getMovieDetailCredit", response);
      dispatch({
        type: TYPE.SET_MOVIE_DETAIL_CAST,
        data: response,
      });
      // console.log('getState().movieReducer: ', getState().movieReducer);
...
export const getMovieDetailVideos = movieID => {
  return async (dispatch, getState) => {
    try {
      const response = await API.getMovieDetailVideos(movieID);
      // console.log("getMovieDetailCredit", response);
      dispatch({
        type: TYPE.SET_MOVIE_DETAIL_VIDEOS,
        data: response.results,
      });
      // console.log('getState().movieReducer: ', getState().movieReducer);
...
export const getMovieSearch = async searchText => {
  // not need dispatch
  try {
    const response = await API.getMovieSearch({query: searchText});
    // console.log('getMovieSearch', response.results);
